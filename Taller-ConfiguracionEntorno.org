...
#+email: correo emilio.alban@epn.edu.ec, juan.auzmontezuma@epn.edu.ec, henry.luisa@epn.edu.ec, anamaria.quillpangui@epn.edu.ec
#+language: Espanol
#+select_tags: export
...
#+begin_src shell :exports both :results verbatim :eval yes
  mamba info
#+end_src
#+RESULTS:
#+begin_example
       ...
       libmamba version : 2.5.0
          mamba version : 2.5.0
           curl version : libcurl/8.19.0 OpenSSL/3.6.1 zlib/1.3.2 zstd/1.5.7 libssh2/1.11.1 nghttp2/1.68.1 mit-krb5/1.22.2
     libarchive version : libarchive 3.8.6 zlib/1.3.2 liblzma/5.8.2 bz2lib/1.0.8 liblz4/1.10.0 libzstd/1.5.7 liblzo2/2.10 openssl/3.5.5 libb2/bundled
       envs directories : /home/henry/miniforge3/envs
          package cache : /home/henry/miniforge3/pkgs
                          /home/henry/.mamba/pkgs
            environment : iccd332 (active)
           env location : /home/henry/miniforge3/envs/iccd332
      user config files : /home/henry/.mambarc
 populated config files : /home/henry/miniforge3/.condarc
       virtual packages : __unix=0=0
                          __linux=4.4.0=0
                          __glibc=2.39=0
               ...
               channels : https://conda.anaconda.org/conda-forge/linux-64
                          https://conda.anaconda.org/conda-forge/noarch
       base environment : /home/henry/miniforge3
               platform : linux-64
#+end_example

*** Estudiante D: Quillupangui Lozano Ana Maria
...
|-----------+----------------------------------------------------------------------+-------------+---------------------------------------|
| ~C-g~     | Cancelar un comando parcialmente introducido                         | ~M-b / M-f~ | Moverse atrás/adelante de una palabra |
| ~C-k~     | Elimina desde el cursor hasta el fin de la línea                     | ~M-w~       | Copiar un segmento de texto           |
| ~C-<SPC>~ | Resaltar un texto entre el cursos y la posición en la que se tecleó  | ~C-/~       | Para deshacer cambios                 |

<Estudiante B> Auz Montezuma Juan Diego
#+ATTR_LATEX: :environment longtable :align |p{0.2\linewidth}|p{0.3\linewidth}|p{0.2\linewidth}|p{0.3\linewidth}|
| *Comando*   | *Descripción*                           | *Comando*     | *Descripción*                         |
|-------------+-----------------------------------------+---------------+---------------------------------------|
| ~C-x C-f~   | Abrir/Guardar archivos                  | ~C-x 1~       | Centrar la pantalla en un solo buffer |
| ~C-a / C-e~ | Moverse al inicio o final de la oración | ~C-space M-w~ | Copiar el texto seleccionado          |
| ~C-y~       | Pegar                                   | ~C-/ o C-x u~ | Revertir cambios                      |

<Estudiante C> Henry Luisa
#+ATTR_LATEX: :environment longtable :align |p{0.2\linewidth}|p{0.3\linewidth}|p{0.2\linewidth}|p{0.3\linewidth}|
| *Comando* | *Descripción*                   | *Comando* | *Descripción*                   |
|-----------+---------------------------------+-----------+---------------------------------|
| ~C-x b~   | Cambiar entre buffers           | ~M-v~     | Desplazarse hacia arriba        |
| ~C-g~     | Cancela un comando en ejecución | ~C-k~     | Borra la linea                  |
| ~C-l~     | Refrescar y centrar la pantalla | ~C-w~     | Corta el contenido seleccionado |

<Estudiante D> Quillupangui Lozano Ana María
#+ATTR_LATEX: :environment longtable :align |p{0.2\linewidth}|p{0.3\linewidth}|p{0.2\linewidth}|p{0.3\linewidth}|
| *Comando*         | *Descripción*                        | *Comando* | *Descripción*                     |
|-------------------+--------------------------------------+-----------+-----------------------------------|
| ~C-x C-c~         |Cerrar Sesión                         | ~C-c C-c~ | Ejecutar acción                   |
| ~M-a / M-e~       |Moverse al final/inicio de la oración | ~C-x 2~   | Dividir Horizontalmente           |
| ~C-x 1~           |Dejar la ventana principal            | ~C-h ?~   |Ayuda general                      |
...
**** Intento 1
[[./images/EmilioAlban-1.png]]
**** Intento 2
[[./images/EmilioAlban-2.png]]
**** Intento 3
[[./images/EmilioAlban-3.png]]
...
**** Intento 1
[[./images/JuanAuz-1.png]]
**** Intento 2
[[./images/JuanAuz-2.png]]
**** Intento 3
[[./images/JuanAuz-3.png]]
Estudiante C: Henry Luisa 
|-----------+---------|
| iteración | Puntaje |
|-----------+---------|
...
|-----------+---------|
**** Intento 1
[[file:images/Henry-1.jpeg]]
**** Intento 2
[[file:images/Henry-2.jpeg]]
**** Intento 3
[[file:images/Henry-3.jpeg]]
#+begin_src org
...
|-----------+---------|
**** Intento 1
[[./images/AnaQuillupangui-1.png]]

**** Intento 2
[[./images/AnaQuillupangui-2.png]]
**** Intento 3
[[./images/AnaQuillupangui-3.png]]
...
**Comandos Fáciles**
1. **Estudiante A: Alban Salazar Emilio Fabian** ~C-<SPC>~ runs the command set-mark-command (found in global-map), which
...
2. **Estudiante B: Auz Montezuma Juan Diego** ~C-/~. C-/ runs the command undo (found in global-map), which is an
interactive native-comp-function in ‘simple.el’. El comando deshace el
último (o últimos) cambios que fueron realizados
3. **Estudiante C: Henry Luisa** ~C-v~C-v runs the command
scroll-up-command (found in global-map), which is an interactive
native-compiled Lisp function in ‘window.el’.   
4. **Estudiante D: Quillupangui Lozano Ana María** ~C-x C-f~ <C-x C-f runs the command find-file (found in global-map), which is an
interactive native-compiled Lisp function in ‘files.el’.>El comando
permite abrir o crear un nuevo documento

**Comandos Difíciles**
1. **Estudiante A: Alban Salazar Emilio Fabian** ~C-h c~ runs the command describe-key-briefly (found in global-map),
...
2. **Estudiante B:Auz Montezuma Juan Diego** ~C-x b~. C-x b runs the command switch-to-buffer (found in global-map), which
is an interactive native-comp-function in ‘window.el’. El comando abre
el menú para cambiar de un buffer a otro
3. **Estudiante C: Henry Luisa** ~C-c C-l~ C-x C-l runs the command
   downcase-region (found in global-map), which is an interactive
   built-in function in ‘C source code’.
4. **Estudiante D:Quillupangui Lozano Ana María** ~C-s~. <C-s runs the command isearch-forward (found in global-map), which is
...
| Luisa Calapiña Henry Paul    | henry.luisa@epn.edu.ec          | colaborador |
| Quillpangui Lozano Ana María | anamaria.quillpangui@epn.edu.ec | colaborador |
| Alban Salazar Emilio Fabian  | emilio.alban@epn.edu.ec         | colaborador |
|                              |                                 |             |
|------------------------------+---------------------------------+-------------|
...
- [X] Generación de Archivo
  PDF ~M-x org-latex-export-to-pdf~
